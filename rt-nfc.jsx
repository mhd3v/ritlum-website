...
const TAP1 = 680, RIPPLE1_END = TAP1 + 600, BEAM1_START = TAP1 + 320, BEAM1_END = TAP1 + 640, FILL1_START = TAP1 + 540, TOAST1_START = TAP1 + 140;
const TAP2 = 2500, RIPPLE2_END = TAP2 + 600, BEAM2_START = TAP2 + 320, BEAM2_END = TAP2 + 640, FILL2_START = TAP2 + 540, TOAST2_START = TAP2 + 140;
const TOAST1_END = 2300, TOAST2_END = 4400, LIFT_START = 3250, LIFT_END = 4400, CYCLE = 5000;
...
const NfcHabitRow = ({ color, name, done, pendingSub }) => (
  <div style={{
    ...
    display: 'grid', gridTemplateColumns: '12px 1fr auto', gap: 11, alignItems: 'center',
    transition: 'box-shadow 200ms ease',
  }}>
    <Dot color={color} size={11} />
    <div style={{ minWidth: 0 }}>
      <div style={{ fontSize: 14.5, color: T.ink, fontWeight: 500, letterSpacing: -0.3 }}>{name}</div>
      <div style={{ fontSize: 11.5, color: T.muted, marginTop: 1 }}>{done ? 'Done · 7:14 AM' : pendingSub}</div>
    </div>
    {done
      ? <CheckPill color={color} />
      : <span style={{ width: 26, height: 26, borderRadius: 99, border: `2px solid ${T.borderStrong}` }} />}
  </div>
);

// compact phone used inside the scene
const NfcScenePhone = React.memo(({ done1, done2, toastOn, toastColor, toastLabel, toastSub }) => (
  <PhoneFrame width={196}>
    <PhoneStatus time="7:14" />
    <div style={{ padding: '8px 16px 0' }}>
      <div style={{ fontSize: 10.5, fontWeight: 600, letterSpacing: 0.5, color: T.muted, textTransform: 'uppercase' }}>Friday</div>
      <h2 style={{ margin: '3px 0 0', fontFamily: T.fontDisplay, fontSize: 26, fontWeight: 700, letterSpacing: -0.9, color: T.ink }}>Today</h2>
    </div>
    <div style={{ padding: '14px 14px 0', display: 'flex', flexDirection: 'column', gap: 8 }}>
      <NfcHabitRow color={T.habit.blue} name="Meditate" done={done1} pendingSub="Tap token to log" />
      <NfcHabitRow color={T.habit.orange} name="Read" done={done2} pendingSub="Tap token to log" />
      <div style={{ background: T.card, borderRadius: 16, padding: '11px 14px', boxShadow: T.shadowSoft,
        display: 'flex', alignItems: 'center', gap: 11, opacity: 0.5 }}>
        <Dot color={T.habit.green} size={10} />
        <span style={{ height: 8, width: 92, borderRadius: 99, background: T.bgAlt }} />
      </div>
    </div>
    {/* completion toast */}
    <div style={{
      position: 'absolute', left: 12, right: 12, bottom: 16,
      background: '#16151a', borderRadius: 16, padding: '11px 13px',
      display: 'flex', alignItems: 'center', gap: 10,
      boxShadow: '0 12px 28px -10px rgba(0,0,0,0.5)',
      transform: toastOn ? 'translateY(0)' : 'translateY(160%)',
      opacity: toastOn ? 1 : 0, transition: 'transform 360ms cubic-bezier(.2,.9,.2,1), opacity 240ms ease',
    }}>
      <CheckPill color={toastColor} />
      <div style={{ minWidth: 0 }}>
        <div style={{ fontSize: 12.5, color: '#fff', fontWeight: 600, letterSpacing: -0.2 }}>{toastLabel}</div>
        <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.55)' }}>{toastSub}</div>
      </div>
    </div>
  </PhoneFrame>
));
...
const NFC_PHASE_INIT = {
  pressed1: false, rippling1: false, beamActive1: false, dotLit1: false, toast1: false, rowGlow1: false,
  pressed2: false, rippling2: false, beamActive2: false, dotLit2: false, toast2: false, rowGlow2: false,
  cycleIdx: 0,
};
const NFC_PHASE_REDUCED = {
  pressed1: false, rippling1: false, beamActive1: false, dotLit1: true, toast1: false, rowGlow1: false,
  pressed2: false, rippling2: false, beamActive2: false, dotLit2: true, toast2: false, rowGlow2: false,
  cycleIdx: 0,
};
...
const NfcAnimation = () => {
  ...
  const [phase, setPhase] = useState(reduced ? NFC_PHASE_REDUCED : NFC_PHASE_INIT);
  ...
  useEffect(() => {
    ...
    const loop = (now) => {
      ...
      const loopOpacity = Math.min(smooth(0, 260, tt), 1 - smooth(CYCLE - 360, CYCLE, tt));
      ...
      const beam1Opacity = beam1Progress * (1 - smooth(BEAM1_END, BEAM1_END + 260, tt));
      const beam2Progress = smooth(BEAM2_START, BEAM2_END, tt);
      const beam2Opacity = beam2Progress * (1 - smooth(BEAM2_END, BEAM2_END + 260, tt));

      // continuous values — straight to the DOM, no React involved
      if (stageRef.current) stageRef.current.style.opacity = loopOpacity.toFixed(3);
      if (phoneRef.current) phoneRef.current.style.transform = `translate(${px}px, ${py}px) rotate(${pr}deg)`;
  ...
  const {
    pressed1, rippling1, beamActive1, dotLit1, toast1, rowGlow1,
    pressed2, rippling2, beamActive2, dotLit2, toast2, rowGlow2, cycleIdx,
  } = phase;
  ...
  const toastSub = toast1 ? 'via habit token · 28-day streak' : 'via habit token · 15-day streak';

  return (
    <div ref={sceneRef} style={{ contain: 'layout paint style', isolation: 'isolate' }}>
      <Scaler w={920} h={540} maxScale={1}>
        <div ref={stageRef} style={{ position: 'absolute', inset: 0, opacity: reduced ? 1 : 0, willChange: 'opacity' }}>
          {/* connecting beams: token 1 → tracker, token 2 → tracker */}
          <div style={{ position: 'absolute', left: 398, top: 153, width: BEAM1_TRAVEL, height: 4, borderRadius: 99,
            transform: 'rotate(23deg)', transformOrigin: 'left center',
            background: `linear-gradient(90deg, ${T.habit.blue}00, ${T.habit.blue}${beam1Alpha})`,
            transition: 'background 200ms ease' }}>
            <span ref={beam1DotRef} style={{ position: 'absolute', top: '50%', left: 0,
              width: 12, height: 12, marginLeft: -6, marginTop: -6, borderRadius: 99,
              background: T.habit.blue, opacity: 0,
              transform: 'translateX(0px)', willChange: 'transform',
              outline: `4px solid ${T.habit.blue}33` }} />
          </div>
          <div style={{ position: 'absolute', left: 398, top: 362, width: BEAM2_TRAVEL, height: 4, borderRadius: 99,
            background: `linear-gradient(90deg, ${T.habit.orange}00, ${T.habit.orange}${beam2Alpha})`,
            transition: 'background 200ms ease' }}>
            <span ref={beam2DotRef} style={{ position: 'absolute', top: '50%', left: 0,
              width: 12, height: 12, marginLeft: -6, marginTop: -6, borderRadius: 99,
              background: T.habit.orange, opacity: 0,
              transform: 'translateX(0px)', willChange: 'transform',
          ...
          <div key={`${cycleIdx}-1`} style={{ position: 'absolute', left: 310, top: 110 }}>
            <NfcToken size={88} color={T.habit.blue} icon="leaf" label="Meditate" rippling={rippling1} pressed={pressed1} />
          </div>
          <div key={`${cycleIdx}-2`} style={{ position: 'absolute', left: 310, top: 320 }}>
            <NfcToken size={88} color={T.habit.orange} icon="book" label="Read" rippling={rippling2} pressed={pressed2} />
          </div>

          {/* tracker */}
          <div style={{ position: 'absolute', left: 520, top: 170 }}>
            <TrackerDevice width={372} rows={rows} highlightRow={highlightRow} />
          </div>

          {/* phone — rendered last so it always sits above the tokens it taps */}
          <div ref={phoneRef} style={{ position: 'absolute', left: 44, top: 44, willChange: 'transform',
            transform: 'translate(0px, 0px) rotate(-7deg)', transformOrigin: '70% 30%' }}>
            <NfcScenePhone done1={dotLit1} done2={dotLit2} toastOn={toastOn}
              toastColor={toastColor} toastLabel={toastLabel} toastSub={toastSub} />
          </div>
        </div>
      </Scaler>
    </div>
...
const M_TAP = 680, M_RIPPLE_END = 1060;
const M_TOAST_START = 820;
const M_BEAM_START = 780, M_BEAM_END = 1320;
const M_FILL_START = 1240, M_FILL_END = 1900;
const M_CHECK_END = 900;
const M_HOLD_END = 2250, M_CYCLE = 3100;
...
const MirrorScenePhone = React.memo(({ done, rippling, pressed, toast }) => (
  <PhoneFrame width={196}>
    ...
    <div style={{ padding: '14px 14px 0' }}>
      ...
      <div style={{
        background: T.card, borderRadius: 18, padding: '14px 14px',
        boxShadow: done ? `0 0 0 2px ${T.habit.orange}55, ${T.shadow}` : T.shadow,
        display: 'grid', gridTemplateColumns: '12px 1fr auto', gap: 11, alignItems: 'center',
        transition: 'box-shadow 200ms ease',
      }}>
        <Dot color={T.habit.orange} size={11} />
        <div style={{ minWidth: 0 }}>
          <div style={{ fontSize: 14.5, color: T.ink, fontWeight: 500, letterSpacing: -0.3 }}>Read</div>
          <div style={{ fontSize: 11.5, color: T.muted, marginTop: 1 }}>{done ? 'Done · 7:14 AM' : '12 / 20 pages'}</div>
        </div>
        <div style={{ position: 'relative', width: 28, height: 28, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          {rippling && (
            <span className="rt-ripple" style={{ position: 'absolute', inset: -6, borderRadius: '50%', border: `2px solid ${T.habit.orange}`, opacity: 0, pointerEvents: 'none' }} />
          )}
          {done
            ? <span style={{ transform: 'scale(1)', transition: 'transform 200ms cubic-bezier(.3,1.4,.6,1)', display: 'inline-flex' }}><CheckPill color={T.habit.orange} /></span>
            : <span style={{ width: 26, height: 26, borderRadius: 99, border: `2px solid ${T.borderStrong}`, background: pressed ? `${T.habit.orange}1a` : 'transparent', transition: 'background 120ms ease' }} />}
        </div>
      </div>
      {/* dimmed neighbours */}
      <div style={{ marginTop: 10, display: 'flex', flexDirection: 'column', gap: 8, opacity: 0.5 }}>
        {[T.habit.blue, T.habit.pink].map((c, i) => (
          <div key={i} style={{ background: T.card, borderRadius: 16, padding: '11px 14px', boxShadow: T.shadowSoft, display: 'flex', alignItems: 'center', gap: 11 }}>
            <Dot color={c} size={10} />
            <span style={{ height: 8, width: i ? 70 : 92, borderRadius: 99, background: T.bgAlt }} />
          </div>
        ))}
      </div>
    </div>
    ...
    <div style={{
      ...
      opacity: toast ? 1 : 0, transition: 'transform 360ms cubic-bezier(.2,.9,.2,1), opacity 240ms ease',
    }}>
      <CheckPill color={T.habit.orange} />
      <div style={{ minWidth: 0 }}>
        <div style={{ fontSize: 12.5, color: '#fff', fontWeight: 600, letterSpacing: -0.2 }}>Read logged</div>
        <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.55)' }}>15-day streak · synced to desk tracker</div>
      </div>
    </div>
  </PhoneFrame>
));

const M_PHASE_INIT = { pressed: false, rippling: false, beamActive: false, done: false, toast: false, dotLit: false, rowGlow: false, cycleIdx: 0 };
...
const MirrorAnimation = () => {
  ...
  const [phase, setPhase] = useState(reduced
    ? { pressed: false, rippling: false, beamActive: false, done: true, toast: false, dotLit: true, rowGlow: false, cycleIdx: 0 }
    : M_PHASE_INIT);
  const phaseRef = useRef(phase);
  ...
  useEffect(() => {
    ...
    const loop = (now) => {
      ...
      const loopOpacity = Math.min(smooth(0, 200, tt), 1 - smooth(M_CYCLE - 260, M_CYCLE, tt));

      const beamProgress = smooth(M_BEAM_START, M_BEAM_END, tt);
      const beamOpacity = beamProgress * (1 - smooth(M_BEAM_END + 20, M_BEAM_END + 180, tt));
      if (stageRef.current) stageRef.current.style.opacity = loopOpacity.toFixed(3);
  ...
  const { pressed, rippling, beamActive, done, toast, dotLit, rowGlow, cycleIdx } = phase;
  ...
  return (
    <div ref={sceneRef} style={{ contain: 'layout paint style', isolation: 'isolate' }}>
      <Scaler w={920} h={500} maxScale={1}>
        <div ref={stageRef} style={{ position: 'absolute', inset: 0, opacity: reduced ? 1 : 0, willChange: 'opacity' }}>
          {/* beam phone → tracker */}
          <div style={{ position: 'absolute', left: 256, top: 246, width: 214, height: 4, borderRadius: 99,
            background: `linear-gradient(90deg, ${T.habit.orange}00, ${T.habit.orange}${beamActive ? '88' : '22'})` }}>
            <span ref={beamDotRef} style={{ position: 'absolute', top: '50%', left: 0, width: 12, height: 12, marginLeft: -6, marginTop: -6, borderRadius: 99, background: T.habit.orange, opacity: 0, transform: 'translateX(0px)', willChange: 'transform', outline: `4px solid ${T.habit.orange}33` }} />
          </div>

          {/* phone */}
          <div key={cycleIdx} style={{ position: 'absolute', left: 60, top: 40 }}>
            <MirrorScenePhone done={done} rippling={rippling} pressed={pressed} toast={toast} />
          </div>

          {/* tracker */}
          <div style={{ position: 'absolute', left: 470, top: 132 }}>
            <TrackerDevice width={372} rows={rows} highlightRow={rowGlow ? 3 : null} />
          </div>
        </div>
      </Scaler>
    </div>
